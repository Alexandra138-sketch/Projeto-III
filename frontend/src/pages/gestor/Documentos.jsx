...
import { Plus, Search, FileText, Download, Loader, AlertTriangle, Trash2 } from 'lucide-react';
...
function GestorDocumentos() {
  ...
  const handleGuardar = async (dados) => {
    try {
      if (modal?.id) {
        const { data } = await api.put(`/documentos/update/${modal.id}`, dados);
        setDocumentos((prev) => prev.map((d) => d.id === modal.id ? { ...d, ...data } : d));
      } else {
        const { data } = await api.post('/documentos/create', dados);
          ...
          <div key={doc.id} className="dash-card incidente-card">
            <div className="d-flex align-items-start gap-3">
              ...
              <div className="d-flex gap-2 flex-shrink-0">
                {doc.ficheiro && (
                  <button className="btn-descarregar" title="Descarregar">
