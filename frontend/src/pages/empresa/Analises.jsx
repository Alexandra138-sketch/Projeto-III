// ─────────────────────────────────────────────────────────────
//  Página: empresa/Analises.jsx
//
//  Página de análises visuais para a conta de empresa.
//  Mostra gráficos dos seus próprios dados:
//    - Incidentes por severidade (donut)
//    - Incidentes por estado (barras)
//    - Incidentes nos últimos 6 meses (linha)
//    - Taxa de resolução (donut)
//    - Documentos por tipo (pizza)
//
//  Usa a biblioteca Recharts (já instalada no projeto).
// ─────────────────────────────────────────────────────────────
...
// Formata uma data como "Jan 2025"
function formatMonthLabel(date) {
  return `${MES_LABELS[date.getMonth()]} ${date.getFullYear()}`;
}

// Converte uma string de data em objeto Date (ou null se inválida)
...
// Gera os labels dos últimos N meses
...
function EmpresaAnalises() {
  ...
  useEffect(() => {
    ...
    // allSettled: se uma chamada falhar, a outra ainda carrega
    Promise.allSettled([
      ...
      const todasFalharam = [incRes, docRes].every(r => r.status === 'rejected');
  ...
  // ── Dados para o gráfico: Incidentes por severidade ──
  ...
  // ── Dados para o gráfico: Incidentes por estado ──
  ...
  // ── Dados para o gráfico: Incidentes por mês ──
  ...
  // ── Dados para o gráfico: Taxa de resolução ──
  ...
  // ── Dados para o gráfico: Documentos por tipo ──
  ...
  return (
    <AdminLayout>
      ...
      <div className="dash-banner">
        ...
        <p>Resumo visual dos incidentes e documentos da tua empresa.</p>
          ...
          <div className="col-6 col-md-4">
            <div className="stat-card">
              <div className="stat-number">
                {carregando ? '…' : incidentes.filter(i => i.estado === 'Aberto' || i.estado === 'A Investigar').length}
              </div>
                    ...
                    <div className="pie-legend">
                      {incidentesPorSeveridade.map(item => (
                        <div className="pie-legend-item" key={item.name}>
                          <span className="pie-dot" style={{ backgroundColor: item.cor }} />
...
export default EmpresaAnalises;
